...
namespace rtp::client
{
    /**
     * @class UISystem
     * @brief System responsible for handling
     * menu interactions
     */
    class UISystem : public ecs::ISystem {
        public:
            UISystem(ecs::Registry &registry, sf::RenderWindow &window,
                     Settings &settings)
                : _registry(registry)
                , _window(window)
                , _settings(settings)
        ...
        private:
            ...
            bool isMouseOverSlider(const ecs::components::ui::Slider &slider,
                                   const sf::Vector2i &mousePos);

            bool isMouseOverDropdown(
                const ecs::components::ui::Dropdown &dropdown,
                const sf::Vector2i &mousePos);

            void updateSliderValue(ecs::components::ui::Slider &slider,
                                   const sf::Vector2i &mousePos);

            int getDropdownOptionAtMouse(
                const ecs::components::ui::Dropdown &dropdown,
                const sf::Vector2i &mousePos);

            bool isMouseOverTextInput(
