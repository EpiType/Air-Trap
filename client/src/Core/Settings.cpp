...
namespace rtp::client
{
    ...
    Settings::Settings()
    {
        ...
        if (!load("config/settings.cfg")) {
            log::warning(
                "No settings.cfg found, using defaults from client.json");
            loadFromClientJson("config/client.json");
        } else {
            log::info("Settings loaded from config/settings.cfg");
        }
    }
    ...
    bool Settings::loadFromClientJson(const std::string &filename)
    {
        ...
        if (!file.is_open()) {
            log::error("Failed to load client.json from: {}", filename);
            return false;
        }
        ...
        while (std::getline(file, line)) {
            ...
            if (inControls) {
                auto parseKeyBinding = [&](const std::string &jsonKey,
                                           KeyAction action) {
        ...
        log::info("Loaded default settings from client.json");
        return true;
    }
    ...
    {
        static const std::unordered_map<sf::Keyboard::Key, std::string>
            keyNames = {
                {sf::Keyboard::Key::A,      "A"     },
                {sf::Keyboard::Key::B,      "B"     },
                {sf::Keyboard::Key::C,      "C"     },
    ...
    bool Settings::save(const std::string &filename)
    {
        ...
        if (!file.is_open()) {
            log::error("Failed to save settings to: {}", filename);
            return false;
        }
        ...
        for (const auto &[action, key] : _keyBindings) {
            file << "key_" << static_cast<int>(action) << "="
                 << static_cast<int>(key) << "\n";
        }

        log::info("Settings saved to: {}", filename);
        return true;
    }
    ...
    bool Settings::load(const std::string &filename)
    {
        std::ifstream file(filename);
        if (!file.is_open()) {
            std::cerr << "Failed to load settings from " << filename
        ...
        while (std::getline(file, line)) {
            ...
            try {
                if (key == "master_volume") {
                    ...
                    _colorBlindMode =
                        static_cast<ColorBlindMode>(std::stoi(value));
                    ...
                    _keyBindings[static_cast<KeyAction>(actionId)] =
                        static_cast<sf::Keyboard::Key>(keyCode);
                }
            } catch (const std::exception &e) {
                std::cerr << "Error parsing setting: " << key << " = " << value
                          << std::endl;
            }
        }
