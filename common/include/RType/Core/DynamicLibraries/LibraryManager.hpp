...
class LibraryManager final {
   public:
     ...
     */
    [[nodiscard]]
    auto load(std::string_view path) noexcept -> std::expected<const DynamicLibrary*, std::string>;
     ...
     */
    [[nodiscard]]
    auto loadShared(std::string_view path) noexcept -> std::expected<std::shared_ptr<DynamicLibrary>, std::string>;
