...
class ThreadPool final {
   public:
     ...
     */
    [[nodiscard]]
    static auto create(size_t numThreads) noexcept -> std::expected<std::unique_ptr<ThreadPool>, std::string>;
    ...
    auto enqueue(F&& f, Args&&... args) noexcept
        -> std::expected<std::future<std::invoke_result_t<F, Args...>>, std::string>;
