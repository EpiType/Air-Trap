...
namespace rtp::thread
{
     ...
     *
     * @details The ThreadPool class creates a fixed number of worker threads
     *          at construction.
     *          Tasks are enqueued to be executed asynchronously,
     *          with each worker thread continuously
     ...
     *          The destructor takes care of shutting down the worker threads
     *          gracefully to ensure proper cleanup of resources.
     */
    class ThreadPool final {
        public:
             ...
             *        threads.
             *
             * @param numThreads The number of worker threads to create in
             *                   the pool.
             * @return std::expected<std::unique_ptr<ThreadPool>, rtp::Error>
             *         A unique pointer to the created ThreadPool on success,
             *         or an error message on failure.
             *
             * @details This static method initializes a ThreadPool instance
             *          with the given number of threads.
             *          It returns a std::expected containing either the
             *          ThreadPool instance or an rtp::Error if the creation
             *          fails.
             */
            [[nodiscard]]
             ...
             *
             * @details Upon destruction, the ThreadPool stops accepting new
             *          tasks and waits for all currently queued tasks to
