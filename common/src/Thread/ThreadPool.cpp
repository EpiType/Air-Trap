...
auto ThreadPool::create(size_t numThreads) noexcept -> std::expected<std::unique_ptr<ThreadPool>, std::string> {
    if (numThreads == 0)
        return std::unexpected{"ThreadPool must have at least one thread."};

    std::unique_ptr<ThreadPool> pool{new (std::nothrow) ThreadPool{}};
    if (!pool)
        return std::unexpected{"ThreadPool init failed: memory allocation error"};
    try {
        log::info("ThreadPool initialized successfully with {} workers.", numThreads);
        pool->start(numThreads);
    } catch (const std::exception& e) {
        std::string err{std::format("ThreadPool init failed: {}", e.what())};
        log::error("{}", err);
...
void ThreadPool::start(size_t numThreads) {
    for (size_t i = 0; i < numThreads; ++i)
        this->_workers.emplace_back([this](std::stop_token st) { this->workerThread(st); });
}
...
void ThreadPool::workerThread(std::stop_token stopToken) noexcept {
    while (true) {
        std::move_only_function<void(void)> task;
        {
            ...
            this->_condition.wait(lock, [this, &stopToken](void) { return this->_stop || !this->_tasks.empty(); });
            if ((stopToken.stop_requested() || this->_stop) && this->_tasks.empty())
                break;
