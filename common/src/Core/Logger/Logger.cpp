...
class LoggerBackend {
   public:
    ...
    void write(Level level, std::string_view msg, const std::source_location& loc) {
        ...
        auto cleanFileName = std::filesystem::path(loc.file_name()).filename().string();
        ...
        std::cout << std::format(
            "{}[{:%T}] [{}] {} \033[0m "
            "\t({}:{})\n",
            color, timePoint, levelStr, msg, loc.file_name(), loc.line());
        if (this->_file.is_open()) {
            this->_file << std::format("[{:%T}] [{}] {} ({}:{})\n", timePoint, levelStr, msg, cleanFileName,
                                       loc.line());
        }
    }
...
auto configure(std::string_view logFilePath) noexcept -> std::expected<void, std::string> {
    try {
        detail::getBackend().setOutputFile(logFilePath);
    } catch (const std::exception& e) {
        return std::unexpected{std::format("Logger configuration failed: {}", e.what())};
    } catch (...) {
